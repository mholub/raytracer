...
#[derive(Clone)]
pub struct Checker(pub Arc<dyn GetColor + Send + Sync>, pub Arc<dyn GetColor + Send + Sync>);

impl Checker {
    pub fn from_colors(color1: Color, color2: Color) -> Self {
        Checker(Arc::new(Texture::from(SolidColor(color1))),
                Arc::new(Texture::from(SolidColor(color2))))
    }
}

impl GetColor for Checker {
    fn value(&self, u: f32, v: f32, p: Point3) -> Color {
